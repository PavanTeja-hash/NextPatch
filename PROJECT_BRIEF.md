...
stays global, because it describes consequences rather than reachability. This was
a deliberate, minimal change to the specified model, chosen because it's the one
that makes the presets meaningfully re-order.

---

## 4b. "Throwaway test machine" is a cap, not a multiplier

**Decision:** a test machine clamps the final score to `5.0` and **skips** the
internet and sensitive-data boosts, rather than applying a `x0.5` discount.

**Why:** as a multiplier it could be cancelled out. Multipliers compose, so
`1.5 x 1.3 x 0.5 = 0.975`, meaning a disposable box scored within 3% of an
unflagged one, which is plainly wrong. A ceiling cannot be defeated by any
combination of ticks.

Simply deleting the `x0.5` and keeping the boosts would be worse: a low-severity
flaw on a test box would then score `base x 1.95`, i.e. *higher* than the same
flaw on an unflagged machine. So the test flag overrides the other two entirely.
The reasoning is that a machine we are willing to wipe does not earn emergency
attention however exposed it is.

**Trade-off accepted:** flaws above the ceiling all tie at `5.0`, so the ranking
sorts on the pre-cap score to keep the worst-first ordering readable.
