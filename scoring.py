...
The model, in four steps:
  ...
  4. ASSET CONTEXT multipliers (only the operator knows these)
       internet-facing   -> * 1.5
       sensitive data    -> * 1.3
       throwaway test box -> * 0.5
       final = base * (every applicable multiplier)

The weights (0.6, 0.4, 1.5, 1.3, 0.5) are a judgement call, NOT science. They
are surfaced as configurable in the UI and would be tuned with a real security
team. They are not presented as authoritative.
"""
...
MULT_TEST = 0.5       # throwaway test machine
...
def score_one(cve_id: str, cvss, epss, on_kev: bool, context: dict,
    ...
    if context.get("test"):
        multipliers.append(("Throwaway test machine", MULT_TEST))
